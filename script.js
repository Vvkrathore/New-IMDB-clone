...
const goToFavouriteButton = document.querySelector("#favourites-button");
...
function favourite(element){
	...
	for(let i = 0; i< currentMovieStack.length; i++){
		if(currentMovieStack[i].id == id){
			let favouriteMovies_ = JSON.parse(localStorage.getItem("favouriteMovies_"));
			
			if(favouriteMovies_ == null){
				favouriteMovies_ = [];
			}

			favouriteMovies_.unshift(currentMovieStack[i]);
			localStorage.setItem("favouriteMovies_", JSON.stringify(favouriteMovies_));
...
goToFavouriteButton.addEventListener('click', ()=>{
	let favouriteMovies_ = JSON.parse(localStorage.getItem("favouriteMovies_"));
	if(favouriteMovies_ == null || favouriteMovies_.length < 1){
		showAlert("you have not added any movie to favourite");
		return;
	}

	currentMovieStack = favouriteMovies_;
	renderList("remove");
})
...
function remove(element){
	...
	for(let i = 0; i<favouriteMovies_.length; i++){
		if(favouriteMovies_[i].id == id){
			continue;
		}
		newFavouriteMovies.push(favouriteMovies_[i]);
	}
	
	localStorage.setItem("favouriteMovies_", JSON.stringify(newFavouriteMovies));
